            ...
            <div class="col-md-10 col-lg-12">
                <div class="card">
                    <div class="card-header">
                        ...
                        <span class="float-right @if (!check('Order Status')->add) d-none @endif">
                            ...
                            <table id="table" class="table table-striped">
                                <thead>
                                    <tr>
                                        ...
                                        <th class="@if (!check('Order Status')->edit && !check('Order Status')->delete) d-none @endif"
                                            id="action">Action</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    @forelse($minfo as $key => $value)
                                        <tr>
                                            ...
                                            <td>{{ $value->name }}</td>
                                            <td>{{ $value->status }}</td>
                                            <td class="text-middle py-0 align-middle @if (!check('Order Status')->edit && !check('Order Status')->delete) d-none @endif">
                                                <div class="btn-group">
                                                    <a href="{{ route('order-status.edit', $value->id) }}"
                                                        class="btn btn-dark btnEdit @if (!check('Order Status')->edit) d-none @endif"><i class="fas fa-edit"></i></a>
                                                    {{-- @endif --}}
                                                    {{-- @if (Auth::user()->can('delete order-status') || Auth::user()->role->id == 1) --}}
                                                    <a href="{{ route('order-status.destroy', $value->id) }}"
                                                        class="btn btn-danger btnDelete @if (!check('Order Status')->delete) d-none @endif"><i class="fas fa-trash"></i></a>
                                                    {{-- @endif --}}
                                                </div>
                                            </td>
                                        </tr>
