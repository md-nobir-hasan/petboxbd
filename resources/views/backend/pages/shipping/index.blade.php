            ...
            <div class="col-md-10 col-lg-12">
                <div class="card">
                    <div class="card-header">
                        ...
                        <span class="float-right @if (!check('Shipping')->add) d-none @endif">
                            ...
                            <table id="table" class="table table-striped">
                                <thead>
                                    <tr>
                                        ...
                                        <th class="@if (!check('Shipping')->edit && !check('Shipping')->delete) d-none @endif"
                                            id="action">Action</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    @forelse($shippings as $key => $value)
                                        <tr>
                                            ...
                                            <td>{{ $value->price }}</td>
                                            <td>{{ $value->status }}</td>
                                            <td class="text-middle py-0 align-middle @if (!check('Shipping')->edit && !check('Shipping')->delete) d-none @endif">
                                                <div class="btn-group">
                                                    <a href="{{ route('shipping.edit', $value->id) }}"
                                                        class="btn btn-dark btnEdit @if (!check('Shipping')->edit) d-none @endif"><i class="fas fa-edit"></i></a>
                                                    {{-- @endif --}}
                                                    {{-- @if (Auth::user()->can('delete shipping') || Auth::user()->role->id == 1) --}}
                                                    <a href="{{ route('shipping.destroy', $value->id) }}"
                                                        class="btn btn-danger btnDelete @if (!check('Shipping')->delete) d-none @endif"><i class="fas fa-trash"></i></a>
                                                    {{-- @endif --}}
                                                </div>
                                            </td>
                                        </tr>
